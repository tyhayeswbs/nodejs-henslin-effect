...
Module creates a die according to a Singleton pattern, with a PhysicsBox
for rendering and simulation, and methods to show either a pre-rendered
animation upon release from the dice cup, or pre-simulate the path of the
die upon release from its final parameters during the shaking period. Either
of these options allow for showing a fixed result on the die.
...
class DieSingleton extends PhysicsBox {
    ...
    /*
        if the die escapes the dice cup prematurely, reset it to its initial position
    */
    resetLocation = () => {
...
class Die extends PhysicsBox {
    ...
    /*
    Teleport the die to position (@x, @y, @z) in world coordinates
    */
    setLocation(x,y,z){
        die = Die.getInstance()
    ...
    */
    ...
    /*
        Check the die is still within the bounds of the dice cup
    */
    ...
    /*
    Find the pre-recorded animation with the closest shake strength (@dv) to the input
    the participant gave, and set the faces on the die so the intended @result is 
    face up at the end of the animation
    */
    ...
    /*
    Pre-simulate the path of the die without rendering it, to determine the face of
    the mesh that is facing up when it comes to rest, and save it into a variable
    for replaying. Set the faces on the die so that the intended @result is 
    face up at the end of the animation
    */
    static simulate_forward(result){
        ...
        while (!(clone_die.velocity.almostZero(1e-6) && clone_die.angularVelocity.almostZero(1e-6) && (clone_die.position.y < -8))){
            ...
            //handle strange vibration condition
    ...
    /*
    Show the correct frame, or an interpolation between two frames of the
    recorded or loaded simulation.  This function is designed to be called
    from the world update hook.
    */
    ...
    /*
    wrapper for attaching the step_recorded_simulation to its hook
    also saves the simulation into input[name="animation"] as a json array
    if such an input exists on the page
    */
    ...
    /*
    Determine which face of @body is currently 'up' for the purposes of
    producing the correct roll result
    */
    static get_up_face(body){
        let localUp = new CANNON.Vec3();
        let inverseDieOrientation = new CANNON.Quaternion()
        let limit = Math.sin(Math.PI /4) //TODO: This could be better tuned
    ...
    /*
    rotate the die so that the @upaxis shows the correct result (@output)
